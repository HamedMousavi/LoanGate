...
public class TabControl {
	 ...
	 */
	private ArrayList<TabItem> mMenuItems;
	 ...
	 * @param ArrayList<TabItem> items
	 * @return void
	 * @throws Exception "Menu list may not be modified while menu is displayed."
	 */	
	public synchronized void setMenuItems(ArrayList<TabItem> items) throws Exception {
		if (mIsShowing) {
	 ...
	 * @return void
	 */	
	public TabControl(Context context, OnMenuItemSelectedListener listener, LayoutInflater lo) {
		mListener = listener;
		mMenuItems = new ArrayList<TabItem>();
		mContext = context;
		mLayoutInflater = lo;
	}
	 ...
	 * @return void
	 */	
	public synchronized void show(View v, int tabLayoutResourceId) {
		mIsShowing = true;
		...
		View mView= mLayoutInflater.inflate(tabLayoutResourceId, null);
        ...
        TableLayout table = (TableLayout)mView.findViewById(R.id.tabs_table);
        ...
        for (int i=0; i < mRows; i++) {
    		...
    		for (int j=0; j< divisor; j++) {
    			...
    			final TabItem cmi = mMenuItems.get(i*divisor+j);
    			View itemLayout = mLayoutInflater.inflate(R.layout.tab_main_item, null);
